        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">{{ old('description', $portfolio->description) }}</textarea>
            </div>

            <div>
                <label for="content" class="block text-sm font-medium text-gray-700">Content</label>
                <textarea name="content" id="content" rows="10"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">{{ old('content', $portfolio->content) }}</textarea>
            </div>
            ...
            <div>
                ...
                @if($portfolio->getFirstMediaUrl('featured_image'))
                    <div class="mb-2">
                        <img src="{{ $portfolio->getFirstMediaUrl('featured_image') }}" alt="Current image" class="h-32 w-auto">
                    </div>
                @endif
                ...
                @if($portfolio->getMedia('images')->count() > 0)
                    <div class="mb-2 grid grid-cols-4 gap-2">
                        @foreach($portfolio->getMedia('images') as $image)
                            <img src="{{ $image->getUrl() }}" alt="Portfolio image" class="h-24 w-full object-cover rounded">
                        @endforeach
                    </div>
                @endif
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" name="order" id="order" value="{{ old('order', $portfolio->order) }}"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                </div>
            ...
            <div>
                ...
                <textarea name="meta_description" id="meta_description" rows="3"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">{{ old('meta_description', $portfolio->meta_description) }}</textarea>
            </div>
