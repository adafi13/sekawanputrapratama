...
<section class="py-5 bg-white border-bottom position-relative overflow-hidden" style="padding-top: 135px !important; padding-bottom: 55px !important;">
  <div class="container text-center position-relative z-2">
    <div class="text-uppercase fw-bold text-success small mb-2" style="letter-spacing: 1.5px; font-size: 11px;">
      • MIKROTIK MULTI-WAN GENERATOR
    </div>
    
    <h1 class="fw-black text-dark display-5 mb-3" style="letter-spacing: -1.2px; font-weight: 900;">
      Load Balance <span class="text-success">PCC Generator</span>
    </h1>
    
    <p class="text-muted mx-auto leading-relaxed mb-0" style="max-width: 720px; font-size: 1.05rem;">
      <strong>Per Connection Classifier (PCC)</strong> membagi lalu lintas berdasarkan *hash* koneksi. Sangat stabil untuk transaksi perbankan, E-Commerce, dan Game Online tanpa terputus.
    </p>
  </div>
</section>
        ...
        <div class="p-4 p-md-5 rounded-4 bg-white border shadow-sm" style="border-color: #e2e8f0 !important;">
          ...
          <form id="pccForm" autocomplete="off">
            ...
            <h5 class="fw-bold text-dark mb-3 pt-2 d-flex align-items-center justify-content-between">
              <span><i class="fas fa-network-wired text-success me-2"></i>Daftar Interface WAN / ISP</span>
              <span class="badge bg-success bg-opacity-10 text-success small fw-normal font-monospace" id="ispCounterBadge">2 ISP Aktif</span>
            </h5>
            ...
            <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 pt-3 border-top">
              <button type="button" class="btn btn-outline-success rounded-pill px-4 fw-bold" id="addIspBtn">
                <i class="fas fa-plus me-1"></i> Tambah Interface ISP
              </button>

              <button type="submit" class="btn btn-success text-white rounded-pill px-5 py-2.5 fw-bold shadow-sm">
                <i class="fas fa-magic me-2"></i> Generate Script PCC
              </button>
            </div>
          </form>
        ...
        <div class="p-4 p-md-5 rounded-4 bg-white border shadow-sm position-relative" style="border-color: #e2e8f0 !important;">
          ...
          <div class="d-flex align-items-center justify-content-between mb-3">
            <h4 class="fw-bold text-success mb-0 d-flex align-items-center gap-2">
              <i class="fas fa-terminal text-success"></i> Hasil Script PCC RouterOS
            </h4>
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('pccForm').addEventListener('submit', function(e) {
      ...
      const script = generatePccScript(version, classifier, ipBlock, ethers, gateways);
  ...
  function generatePccScript(version, classifier, ipBlock, ethers, gateways) {
    const n = ethers.length;
    const now = new Date();
    const dateStr = now.toLocaleString('id-ID');

    let script = "###############################################################\n";
    script += "# MIKROTIK LOAD BALANCE PCC (Per Connection Classifier) SCRIPT\n";
    ...
    script += "# Classifier Hash: " + classifier + "\n";
    ...
    ipBlocks.forEach(block => {
      script += `add address=${block} list=ip-local comment="LB PCC by Sekawan"\n`;
    });
    script += "\n";
    ...
    for (let i = 0; i < n; i++) {
      script += `add chain=srcnat out-interface="${ethers[i]}" action=masquerade comment="LB PCC WAN ${i + 1} by Sekawan"\n`;
    }
    script += "\n";
    ...
    if (version === "7") {
      ...
      for (let i = 0; i < n; i++) {
        script += `add name="to-${ethers[i]}" fib comment="LB PCC Table by Sekawan"\n`;
      }
      script += "\n/ip route\n";
      for (let i = 0; i < n; i++) {
    ...
    ["prerouting", "postrouting", "forward", "input", "output"].forEach(chain => {
      script += `add action=accept chain=${chain} dst-address-list=ip-local src-address-list=ip-local comment="LB PCC Bypass Local Traffic"\n`;
    });

    // Mark Connection (Input & Prerouting PCC)
    for (let i = 0; i < n; i++) {
      script += `add action=mark-connection chain=input in-interface="${ethers[i]}" new-connection-mark="cm-${ethers[i]}" passthrough=yes comment="LB PCC Input ${ethers[i]}"\n`;
    }
