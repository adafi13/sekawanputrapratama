...
@section('title', 'Generator Script Load Balance NTH MikroTik RouterOS v6 Gratis - PT Sekawan Putra Pratama')
@section('meta_description', 'Buat script load balance MikroTik metode NTH (Every-Packet Round Robin) otomatis untuk RouterOS v6 hingga 10 ISP. Gratis, akurat, siap paste di Winbox Terminal.')
@section('meta_keywords', 'script load balance nth mikrotik, nth routeros, load balance round robin mikrotik, multi wan mikrotik v6, script nth routeros v6, generator script mikrotik gratis, every packet load balance mikrotik, pembagian bandwidth mikrotik')
@section('og_title', 'Generator Script Load Balance NTH MikroTik - PT Sekawan Putra Pratama')
@section('og_description', 'Generator script NTH Round Robin untuk MikroTik RouterOS v6. Isi form ISP, klik generate, salin ke Winbox Terminal. Gratis dan akurat.')
...
<section class="py-5 bg-white border-bottom position-relative overflow-hidden" style="padding-top: 135px !important; padding-bottom: 55px !important;">
  <div class="container text-center position-relative z-2">
    <div class="text-uppercase fw-bold text-info small mb-2" style="letter-spacing: 1.5px; font-size: 11px;">
      • MIKROTIK MULTI-WAN GENERATOR
    </div>
    
    <h1 class="fw-black text-dark display-5 mb-3" style="letter-spacing: -1.2px; font-weight: 900;">
      Load Balance <span class="text-info">NTH Generator</span>
    </h1>
    
    <p class="text-muted mx-auto leading-relaxed mb-0" style="max-width: 720px; font-size: 1.05rem;">
      <strong>NTH (Every-Packet / Round-Robin)</strong> membagi urutan paket secara bergantian ke setiap ISP. Sangat cocok untuk mengoptimalkan akumulasi kecepatan *download* besar.
    </p>
  </div>
</section>
        ...
        <div class="p-4 p-md-5 rounded-4 bg-white border shadow-sm" style="border-color: #e2e8f0 !important;">
          ...
          <form id="nthForm" autocomplete="off">
            <div class="row g-3 mb-4">
              ...
              <div class="col-md-8">
                ...
                <span class="text-muted small mt-1 d-block" style="font-size: 11px;">
                  Masukkan blok IP lokal yang akan di-routing (pisahkan koma untuk banyak subnet).
                </span>
              </div>
            ...
            <h5 class="fw-bold text-dark mb-3 pt-2 d-flex align-items-center justify-content-between">
              <span><i class="fas fa-network-wired text-info me-2"></i>Daftar Interface WAN / ISP</span>
              <span class="badge bg-info bg-opacity-10 text-info small fw-normal font-monospace" id="ispCounterBadge">2 ISP Aktif</span>
            </h5>
            ...
            <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 pt-3 border-top">
              <button type="button" class="btn btn-outline-info rounded-pill px-4 fw-bold" id="addIspBtn">
                <i class="fas fa-plus me-1"></i> Tambah Interface ISP
              </button>

              <button type="submit" class="btn btn-info text-white rounded-pill px-5 py-2.5 fw-bold shadow-sm">
                <i class="fas fa-magic me-2"></i> Generate Script NTH
              </button>
            </div>
          </form>
        ...
        <div class="p-4 p-md-5 rounded-4 bg-white border shadow-sm position-relative" style="border-color: #e2e8f0 !important;">
          ...
          <div class="d-flex align-items-center justify-content-between mb-3">
            <h4 class="fw-bold text-success mb-0 d-flex align-items-center gap-2">
              <i class="fas fa-terminal text-success"></i> Hasil Script NTH RouterOS
            </h4>
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('nthForm').addEventListener('submit', function(e) {
      ...
      const script = generateNthScript(version, ipBlock, ethers, gateways);
  ...
  function generateNthScript(version, ipBlock, ethers, gateways) {
    const n = ethers.length;
    const now = new Date();
    const dateStr = now.toLocaleString('id-ID');

    let script = "###############################################################\n";
    script += "# MIKROTIK LOAD BALANCE NTH SCRIPT\n";
    ...
    script += "# Jumlah ISP: " + n + " WAN Link (Every-Packet Round Robin)\n";
    script += "#\n";
    ...
    ipBlocks.forEach(block => {
      script += `add address=${block} list=ip-local comment="LB NTH by Sekawan"\n`;
    });
    script += "\n";
    ...
    ["prerouting", "postrouting", "forward", "input", "output"].forEach(chain => {
      script += `add action=accept chain=${chain} dst-address-list=ip-local src-address-list=ip-local comment="LB NTH Bypass Local Traffic"\n`;
    });

    // Mark Connection (Input & Prerouting NTH)
    for (let i = 0; i < n; i++) {
      script += `add action=mark-connection chain=input in-interface="${ethers[i]}" new-connection-mark="cm-${ethers[i]}" passthrough=yes comment="LB NTH Input ${ethers[i]}"\n`;
    }
